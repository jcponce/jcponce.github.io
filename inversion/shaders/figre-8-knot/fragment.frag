...
const int kNum = 128; // subdivisions. make 1024 on fast machines
const float kRad = 0.06; // thickness

// knot
vec3 knot( in float t ) {
    t *= 6.283185; // knot in S3
    const float e = 0.16;
    const float h = 0.25;
    float a = e*sin(4.0*t);
    float b = 1.0-a*a;
    vec4 q = vec4( b*(h*cos(t)+(1.0-h)*cos(3.0*t)),
                   b*(2.0*sqrt(h-h*h)*sin(2.0*t)),
                   a*(2.0),
                   b*(h*sin(t)-(1.0-h)*sin(3.0*t)) ) / (1.0+a*a);
...
vec4 iCylinder( in vec3 ro, in vec3 rd, in vec3 pa, in vec3 pb, float ra, out float v ) {
    vec4 res = vec4(-1.0);
    v = 0.0;
...
// https://iquilezles.org/articles/intersectors
bool sCylinder( in vec3 ro, in vec3 rd, in vec3 pa, in vec3 pb, float ra ) {
    vec3 ba = pb-pa;
    vec3 oc = ro-pa;
    float baba = dot(ba,ba);
    float bard = dot(ba,rd);
...
// do coloring and lighting - MODIFIED: Removed white stripes
// vec3 shade( in vec3 pos, in vec3 nor, in vec3 rd, in float hm ) {
//     // material - pure HSV rainbow colors (no white stripes)
//     vec3 mate = 0.5 + 0.5*cos(hm*6.283185*3.0 + vec3(0.0, 2.0, 4.0) + iTime*0.5);
...
void main() {
    ...
    for( int n=0; n<AA; n++ ) {
        ...
        vec2 p = uv + o * (2.0 / iResolution.xy);
    #else
        vec2 p = uv;
    #endif
        ...
        // background - dark blue instead of black for better contrast
        vec3 col = vec3(0.05, 0.08, 0.12)*(1.0-0.15*dot(p,p))*smoothstep(-1.0,1.0,rd.y);
        ...
        // tint - removed to preserve pure HSV colors
