...
class Boid {
  ...
  // ---- GLOW LOGIC FOR PINK BOIDS ----
  let glow = false;
  if (this.n == -4) {
    let d = this.getNearestGreenBoidDistance();
    if (d < 80) glow = true;
  }

  if (glow) {
    drawingContext.shadowBlur = 25;
    drawingContext.shadowColor = "rgba(255,100,200,1)";
    stroke(255, 150, 210); // bright pink while glowing
  } else {
  ...
  for (let i = 0; i <= 360; i += 5) {
    ...
    let nextx = this.sz * size * (sin(this.a * t) + sin(Math.sign(this.a - this.b) * this.b * t) + sin(this.n * t));
    let nexty = this.sz * size * (cos(this.a * t) + cos(Math.sign(this.a - this.b) * this.b * t) + cos(this.n * t));
    vertex(nextx, nexty);
  }
  endShape();
  pop();

  // Important: turn off glow
  drawingContext.shadowBlur = 0;
}
