 ...
 * Written by Juan Carlos Ponce Campuzano, 9-Oct-2018
...
function draw() {
    ...
    for (let a = 1; a <= gridsize; a++) {
        for (let b = 1; b <= gridsize; b++) {
            ...
            let t = 0;
            ox = cx + r * (sin(1.0 * a * t) + pow(sin(1.0 * b * t), 2)) * cos(t);
            oy = cy + r * (sin(1.0 * a * t) + pow(sin(1.0 * b * t), 2)) * sin(t);
            
            let done = false;
            while (t < 4 * gridsize * TWO_PI && !done) {
                
                x = cx + r * (sin(1.0 * a * t) + pow(sin(1.0 * b * t), 2)) * cos(t);
                y = cy + r * (sin(1.0 * a * t) + pow(sin(1.0 * b * t), 2)) * sin(t);
